...
$error_string = "";
$user_data['acnt_type'] = "Null";     
if($_SERVER['REQUEST_METHOD'] == "POST"){
     $user_name = $_POST['uname'];
     $mail = ($_POST['email']);
     $acnt_type = $_POST['accnt_select'];
     if(!empty($user_name) && !empty($mail) && !is_numeric($user_name)){
          ...
          if($acnt_type == 'admin'){
               ...
               if($result){
                    if($result && mysqli_num_rows($result) > 0)   {
                         ...
                         if($user_data['email'] === $mail && $user_data['acnt_type'] === $acnt_type){
                              $_SESSION['flag'] = 1;
                              $_SESSION['selected'] = $user_data['user_id'];
                              header("Location: forgot_psw_mailer.php");
                              die;
                         }else{ $error_string = "Account not found: Wrong Username or Email ID";}
                    }
               }if ($user_data['acnt_type'] != $acnt_type) {
                    $error_string = "Account not found:  wrong type";
               }else{    $error_string ="Account not found: Wrong Username or Email ID";      }
               ...
               if ($user_data['acnt_type'] != $acnt_type) {
                    $error_string = "Account not found: wrong type";
               }else{    $error_string="Account not found: Wrong User name or Email ID";}
          ...
          <div class="container" style="padding-top:-100px;">
            <form method="post" class="formcontainer">
                ...
                <div class="logincontainer">
                    <h2 ALIGN="CENTER" style="font-family: 'Nunito', sans-serif;">Reset Password</h2>
