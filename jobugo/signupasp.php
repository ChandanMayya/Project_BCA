...
if($_SERVER['REQUEST_METHOD'] == "POST")
{
	...
	$password = ($_POST['psw']);
     ...
     $_SESSION['flag'] = 1; //Used in validation page
                         ...
                         if($password == $password_rep){
                              if(strlen($user_name)>4){
                                   ...
                                   if(mysqli_num_rows($resultt) == 0){
                                        ...
                                        $stmt = $con->prepare("insert into users (user_id,user_name,email,password,acnt_type,conf_doc,mime) values (?,?,?,?,?,?,?)");
                                        $stmt->bind_param('sssssss',$uid,$user_name,$mail,$password,$acnt_type,$data,$mime);
                                        if($stmt->execute()){
    ...
    <section>
          <div class="container">
               <form method = "post" class="formcontainer" enctype="multipart/form-data">
                  ...
                  <center>
               <label for="file_upload"><b>Upload your Resume / CV in PDF format with file size less than 16MB</b></label><br>
